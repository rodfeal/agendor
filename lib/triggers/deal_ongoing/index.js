...
const organization = (plg, event, organization) => {
  if (organization) {
    return plg.axios({
      ...
      res.data.data.products = res.data.data.products.map(p => p.name).join(', ');
...
const person = (plg, event, person) => {
  if (person) {
    return plg.axios({
      ...
      people.products = people.products.map(p => p.name).join(', ');
      people.birthday = people.birthday ? people.birthday.substr(5).split('-').reverse().join('/') : null;
      return people;
    });
  }
  return null;
